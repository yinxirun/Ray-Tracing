...
void VulkanBase::initVulkan(Context *context)
{
    auto instanceExtensions = getRequiredExtensions(context->window.get());

    volkInitialize();
    createInstance(instanceExtensions);
    volkLoadInstance(instance);
    setupDebugMessenger();
    createSurface(context->window.get());
    pickPhysicalDevice();
    createLogicalDevice();
...
void VulkanBase::recreateSwapChain(int width, int height)
{
...
void VulkanBase::createInstance(std::vector<const char *> &instanceExtensions)
{
...
void VulkanBase::createSwapChain(int widht, int height)
{
    SwapChainSupportDetails swapChainSupport = querySwapChainSupport(physicalDevice);

    VkSurfaceFormatKHR surfaceFormat = chooseSwapSurfaceFormat(swapChainSupport.formats);
    VkPresentModeKHR presentMode = chooseSwapPresentMode(swapChainSupport.presentModes);
    VkExtent2D extent = chooseSwapExtent(swapChainSupport.capabilities, widht, height);
...
VkExtent2D VulkanBase::chooseSwapExtent(const VkSurfaceCapabilitiesKHR &capabilities, int width, int height)
{
...
std::vector<const char *> VulkanBase::getRequiredExtensions(Window *window)
{
    uint32_t extensionCount = 0;
    const char **extension_names;
    extension_names = window->GetRequiredInstanceExtensions(&extensionCount);

    std::vector<const char *> extensions(extension_names, extension_names + extensionCount);
